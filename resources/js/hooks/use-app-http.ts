...
>(data: TForm | (() => TForm), defaults: UseAppHttpOptions = {}) {
  ...
  return {
    ...
    post: (url: string, options?: AppHttpRequestOptions<TResponse, TForm>) =>
    ...
    patch: (url: string, options?: AppHttpRequestOptions<TResponse, TForm>) =>
      request.patch(url, withDefaults(options)),
    delete: (url: string, options?: AppHttpRequestOptions<TResponse, TForm>) =>
