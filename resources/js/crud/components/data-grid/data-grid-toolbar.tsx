...
export function DataGridSelectionToolbar({
  ...
  const [selectedAction, setSelectedAction] = useState<CrudRecordAction | null>(
    null,
  );
...
export function DataGridToolbar({
  ...
  const filterFields: FilterFieldConfig<string>[] = resource.columns
    .filter(
      (column) => !column.detailOnly && column.filterable && column.filter,
    )
    .map((column) => ({
      key: column.name,
      label: column.label,
      ...
      options:
        column.type === 'boolean'
          ? booleanOptions
          : Object.entries(column.options ?? {}).map(([value, option]) => ({
              value,
        ...
        {showFilters && (
          <Filters
            ...
            onChange={(nextFilters) => {
              onConditionsChange(
                nextFilters.map(({ field, operator, values }) => ({
                  field,
                  operator,
                  values,
                })),
