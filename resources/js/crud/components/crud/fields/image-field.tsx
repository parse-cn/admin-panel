...
export function ImageField({
  ...
  const upload = useHttp<{ file: File | null }, Response>({ file: null });
  useEffect(
    () => () => {
  ...
  return (
    <FileUpload
      ...
      labels={{
        ...
        uploading: (progress) => t('crud.upload.uploading', { progress }),
        remove: t('crud.upload.remove'),
      }}
      previewUrl={preview || currentUrl || undefined}
      progress={
        upload.processing ? (upload.progress?.percentage ?? 0) : undefined
      }
      onFileSelect={select}
      onClear={
        value || preview
