...
export function PreferencesContent() {
  ...
  const [layout, setLayout] = useState<Layout>(() =>
    ...
    (window.localStorage.getItem('parse-admin-panel-app-shell') ===
      ...
      window.localStorage.getItem('parse-admin-panel-app-shell') ===
        'app-shell-3' ||
      window.localStorage.getItem('parse-admin-panel-app-shell') === 'default')
      ? (window.localStorage.getItem('parse-admin-panel-app-shell') as Layout)
      : props.panel.appShell,
  );
  const [brandColor, setBrandColor] = useState<BrandColor>(() => {
    if (typeof window === 'undefined') return 'default';
    const value = window.localStorage.getItem('parse-admin-panel-brand-color');
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('parse-admin-panel-brand-color', brandColor);
  }, [brandColor]);
          ...
          >
            ...
            <SelectContent>
              {supportedLocales.map((supported) => (
                <SelectItem key={supported.locale} value={supported.locale}>
                  {supported.label}
                </SelectItem>
              ))}
            </SelectContent>
