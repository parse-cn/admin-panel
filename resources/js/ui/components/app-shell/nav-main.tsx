      ...
      <SidebarGroup className={cn('px-2 pb-0', className)}>
        <SidebarGroupContent>
          <GlobalSearch
            ...
            minimumQueryLength={navigation.globalSearch.minimumQueryLength}
            ...
            <SidebarMenu>
              {section.groups.map((group) =>
                group.items.length === 1 ? (
                  ...
                  <NavigationModule group={group} key={group.label} />
                ),
              )}
            </SidebarMenu>
          </SidebarGroupContent>
    ...
    return (
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger
            render={
              <SidebarMenuButton isActive={isActive} tooltip={group.label} />
            }
          >
            <RemixIcon
              className={isActive ? 'text-primary opacity-100' : undefined}
              name={icon}
            />
          ...
          >
            <DropdownMenuGroup>
              ...
              {group.items.map((item) => (
                <DropdownMenuItem
                  className={
                    item.active ? 'bg-accent text-accent-foreground' : undefined
                  }
                  key={item.href}
                  render={<Link href={item.href} />}
                >
