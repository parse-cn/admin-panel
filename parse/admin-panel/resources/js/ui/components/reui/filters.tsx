...
export interface FilterFieldConfig<T = unknown> {
    ...
    renderOptionList?: (
        props: FilterOptionListRenderProps<T>,
    ) => React.ReactNode;
...
const createOperatorsFromI18n = (
    ...
    number: [
        { value: 'is', label: i18n.operators.equals },
...
function FilterOperatorDropdown<T = unknown>({
    ...
    if (field.operatorSelectable === false) {
        return <ButtonGroupText>{operatorLabel}</ButtonGroupText>;
    }
                            ...
                            if (e.key === 'ArrowDown') {
                                ...
                                if (allFilteredOptions.length > 0) {
                                    setHighlightedIndex((prev) =>
                                        prev < allFilteredOptions.length - 1
                                            ? prev + 1
                                            : 0,
                                    );
                                }
                                ...
                                e.preventDefault();
                                const option =
                                    allFilteredOptions[highlightedIndex];
                                if (option) {
                ...
                >
                    {isAsync && loading && allFilteredOptions.length === 0 ? (
                        <div className="py-2 text-center text-sm text-muted-foreground">
                            {context.i18n.loadingOptions ??
                                DEFAULT_I18N.loadingOptions}
                        </div>
                    ) : isAsync && error ? (
                        <div className="py-2 text-center text-sm text-muted-foreground">
...
interface DateFilterInputProps<T = unknown> {
    values: T[];
    onChange: (values: T[]) => void;
    operator: string;
    autoFocus?: boolean;
...
function DateFilterInput<T = unknown>({
    values,
    onChange,
    operator,
    autoFocus,
    ...
    useEffect(() => {
        ...
        setDate(parseDate(values[0]));
        setRange({ from: parseDate(values[0]), to: parseDate(values[1]) });
    }, [open, values]);
    ...
    const apply = () => {
        ...
        if (!isRange && date) {
            onChange([format(date, 'yyyy-MM-dd')] as T[]);
        }
            ...
            return (
                <div className="flex items-center gap-1">
                    <FilterInput
                        ...
                        value={(values[0] as string) || ''}
                        onChange={(e) =>
                            onChange([e.target.value, values[1] || ''] as T[])
                        }
                    ...
                    <FilterInput
                        ...
                        onChange={(e) =>
                            onChange([values[0] || '', e.target.value] as T[])
                        }
